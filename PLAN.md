...
# Add new token "<pref_42>" to tokenizer + embedding table
placeholder = "<pref_42>"
num_added = tokenizer.add_tokens([placeholder])
text_encoder.resize_token_embeddings(len(tokenizer))
placeholder_id = tokenizer.convert_tokens_to_ids(placeholder)

# Initialize new row from something reasonable (e.g. average of descriptive words)
init_words = ["woman", "portrait", "person"]
init_ids = tokenizer.convert_tokens_to_ids(init_words)
with torch.no_grad():
    init_vec = text_encoder.get_input_embeddings().weight[init_ids].mean(0)
    text_encoder.get_input_embeddings().weight[placeholder_id] = init_vec

# Only the new row is trainable
embedding_layer = text_encoder.get_input_embeddings()
embedding_layer.weight.requires_grad_(True)
# (Extra: mask out gradient for all other rows, see "Gradient masking" below)
...
for step in range(num_steps):
    ...
    pred_prompt   = template.format(s=placeholder)

    # Tokenize both (pad to 77)
    target_ids = tokenizer(target_prompt, padding="max_length",
                           max_length=77, truncation=True,
                           return_tensors="pt").input_ids.to("mps")
    pred_ids   = tokenizer(pred_prompt, padding="max_length",
                           max_length=77, truncation=True,
                           return_tensors="pt").input_ids.to("mps")

    # Random latent and timestep
    z = torch.randn(1, 4, 64, 64, device="mps")
    ...
    # Prediction path — keeps gradient through the pseudo-token
    ...
    optimizer.zero_grad()
    loss.backward()
    # Mask grad on all rows except placeholder (see below)
    with torch.no_grad():
